...
impl Storage {
    ...
    pub fn list_commands(&self) -> Result<Vec<Command>, String> {
        ...
        let mut stmt = conn
            .prepare("SELECT id, name, command, terminal, auto_start, group_name, created_at, updated_at FROM commands ORDER BY group_name, created_at DESC")
    ...
    pub fn get_command(&self, id: &str) -> Result<Command, String> {
        let conn = self.conn.lock().map_err(|e| format!("Lock error: {}", e))?;
        conn.query_row(
            "SELECT id, name, command, terminal, auto_start, group_name, created_at, updated_at FROM commands WHERE id = ?1",
            params![id],
            |row| {
                Ok(Command {
                    id: row.get(0)?,
    ...
    pub fn add_command(&self, cmd: &Command) -> Result<(), String> {
        let conn = self.conn.lock().map_err(|e| format!("Lock error: {}", e))?;
        conn.execute(
            "INSERT INTO commands (id, name, command, terminal, auto_start, group_name, created_at, updated_at) VALUES (?1,?2,?3,?4,?5,?6,?7,?8)",
            params![
    ...
    pub fn update_command(&self, cmd: &Command) -> Result<(), String> {
        ...
        let affected = conn
            .execute(
                "UPDATE commands SET name=?1, command=?2, terminal=?3, auto_start=?4, group_name=?5, updated_at=?6 WHERE id=?7",
                params![
    ...
    pub fn get_auto_start_commands(&self) -> Result<Vec<Command>, String> {
        let conn = self.conn.lock().map_err(|e| format!("Lock error: {}", e))?;
        let mut stmt = conn
            .prepare("SELECT id, name, command, terminal, auto_start, group_name, created_at, updated_at FROM commands WHERE auto_start = 1 ORDER BY created_at DESC")
            .map_err(|e| format!("Failed to prepare query: {}", e))?;
    ...
    pub fn get_group_commands(&self, group: &str) -> Result<Vec<Command>, String> {
        let conn = self.conn.lock().map_err(|e| format!("Lock error: {}", e))?;
        let mut stmt = conn
            .prepare("SELECT id, name, command, terminal, auto_start, group_name, created_at, updated_at FROM commands WHERE group_name = ?1 ORDER BY created_at ASC")
            .map_err(|e| format!("Failed to prepare query: {}", e))?;
